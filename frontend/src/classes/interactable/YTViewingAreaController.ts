import { YTViewingArea as YTViewingAreaModel } from '../../types/CoveyTownSocket';
...
/**
 * The events that a YTViewingAreaController can emit
 */
export type YTViewingAreaEvents = BaseInteractableEventMap & {
  /**
...
export default class ViewingAreaController extends InteractableAreaController<
  ...
  YTViewingAreaModel
> {
  private _model: YTViewingAreaModel;

  /**
   * Constructs a new ViewingAreaController, initialized with the state of the
   * provided viewingAreaModel.
   *
   * @param ytviewingAreaModel The viewing area model that this controller should represent
   */
  constructor(ytviewingAreaModel: YTViewingAreaModel) {
    super(ytviewingAreaModel.id);
    this._model = ytviewingAreaModel;
  }
  ...
  /**
   * The queue of videos - as a list of strings.
   */
  public get queue() {
  ...
  /**
   * The like on videas - true if liked, false otherwise.
   */
  public get like() {
    return this._model.like;
  }
   ...
   * Changing this value will emit a 'queueChange' event to listeners
   */
  public set like(like: boolean) {
    if (this._model.like != like) {
      this._model.like = like;
      this.emit('likeChange', like);
    }
  }
   ...
   * @param updatedModel
   */
  protected _updateFrom(updatedModel: YTViewingAreaModel): void {
