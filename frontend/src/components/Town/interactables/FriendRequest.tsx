...
function InviteFriendComponent(): JSX.Element {
  ...
  useEffect(() => {
    const findUser = async () => {
      ...
      if (userInfo.friends.includes(currentUser.id)) {
        setShowInvite(true);
      }
      if (currentUser.sentRequests.includes(userInfo.id)) {
  ...
  return (
    <Flex width='full' align='center' justifyContent='center'>
      <Box p={2} maxWidth='500px' w='90vw' h='100vh' mt={50} class='box-profile'>
